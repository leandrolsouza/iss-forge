...
function makeTeam(overrides = {}) {
  return {
    ...
    uniforms: {
      home: { shirt: [{ r: 248, g: 0, b: 0 }, { r: 200, g: 0, b: 0 }, { r: 150, g: 0, b: 0 }] },
    },
    ...overrides,
  };
}
...
describe('validateRom', () => {
  describe('Player Name Validation', () => {
    ...
    it('should detect name exceeding 8 characters', () => {
      ...
      const warnings = validateRom(teams);
      expect(warnings.some(w => w.type === 'error' && w.message.includes('excede 8 caracteres'))).toBe(true);
    });
  ...
  describe('Duplicate Numbers', () => {
    it('should detect duplicate player numbers in same team', () => {
      ...
      const warnings = validateRom(teams);
      expect(warnings.some(w => w.type === 'warning' && w.message.includes('duplicado'))).toBe(true);
    });
  ...
  describe('Stats Out of Range', () => {
    it('should detect shooting below 1', () => {
      ...
      const warnings = validateRom(teams);
      expect(warnings.some(w => w.type === 'error' && w.message.includes('chute invalido'))).toBe(true);
    });
    ...
    it('should detect speed below 1', () => {
      ...
      const warnings = validateRom(teams);
      expect(warnings.some(w => w.type === 'error' && w.message.includes('velocidade invalida'))).toBe(true);
    });
    ...
    it('should detect stamina below 1', () => {
      ...
      const warnings = validateRom(teams);
      expect(warnings.some(w => w.type === 'error' && w.message.includes('stamina invalida'))).toBe(true);
    });
  ...
  describe('Uniform Color Warnings', () => {
    it('should warn when all home shirt colors are black (0,0,0)', () => {
      const teams = [makeTeam({
        uniforms: {
          home: {
            shirt: [{ r: 0, g: 0, b: 0 }, { r: 0, g: 0, b: 0 }, { r: 0, g: 0, b: 0 }],
          },
        },
      })];
      const warnings = validateRom(teams);
    ...
    it('should not warn when at least one shirt color is not black', () => {
      const teams = [makeTeam({
        uniforms: {
          home: {
            shirt: [{ r: 0, g: 0, b: 0 }, { r: 255, g: 0, b: 0 }, { r: 0, g: 0, b: 0 }],
          },
        },
      })];
      const warnings = validateRom(teams);
