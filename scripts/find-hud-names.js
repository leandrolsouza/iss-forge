...
const romPath =
  process.argv[2] ||
  'E:\\Desenvolvimento\\Games\\ISS-Editor-IA\\rom\\International Superstar Soccer (Europe).sfc';
...
['IRELAND', 'GERMANY', 'BRAZIL'].forEach((name) => {
  ...
  for (const ch of name) {
    pattern.push(0x6c + (ch.charCodeAt(0) - 65));
    pattern.push(0x00);
  }
  const r = search(pattern);
  console.log(
    `${name}: ${r.length > 0 ? r.map((x) => '0x' + x.toString(16)).join(', ') : 'NOT FOUND'}`,
  );
});
...
for (let i = 0; i < rom.length - 7; i++) {
  ...
  if (match && rom[i] > 0x01 && rom[i] < 0xf0) {
    // reasonable first byte
    found.push(i);
  }
}
console.log(`Pattern matches: ${found.length}`);
found.slice(0, 10).forEach((addr) => {
  const bytes = Array.from(rom.slice(addr, addr + 10))
  ...
  for (let base = 0; base < 256; base++) {
    ...
    for (let i = 0; i < rom.length - name.length * 2; i++) {
      ...
      if (match) {
        matchCount++;
        if (matchCount <= 2) {
          const bytes = Array.from(rom.slice(i, i + name.length * 2 + 2))
            .map((b) => b.toString(16).padStart(2, '0'))
            .join(' ');
